        ...
        Page.loadEventFired(() => {
          ...
          checkIfReady(Runtime, () => {
            getHtmlContent(Runtime).then((result) => {
              ...
              if (Spiderable.keepMeteorJsResource) {
                html = html.replace(/<script(?!([^>]*?)keep|meteor_js_resource=true)(?:[^>]+?)>(.|\n|\r)*?<\/script\s*>/ig, "");
              } else {
...
WebApp.connectHandlers.use((req, res, next) => {
  if ((/\?.*_escaped_fragment_=/.test(req.url) || Spiderable.userAgentRegExps.some(re => {
    ...
    if (cached) {
      if (res) {
        // Todo: generalize the handling of non html contents , here and in pageCacher.
        isCss = url.includes(".css");
        if (!isCss) {
          res.writeHead(200, {"Content-Type": "text/html; charset=UTF-8"});
        } else {
